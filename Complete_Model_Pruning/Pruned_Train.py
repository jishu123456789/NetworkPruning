...
def train():
    ...
    pruned_resnet_model_path = r"fine_tuned_model.pth"
    ...
    model = New_Pruned_Model.EncodertoDecoder(embeding_size=embedding_size,
                            trg_vocab_size=trg_vocab_size,
                            num_heads=num_heads,
                            num_decoder_layers=num_decoder_layers,
                            dropout=dropout , pruned_resnet_model_path = pruned_resnet_model_path).to(device)
    # Load the entire model instance
    #model_instance = torch.load('Only_ResNet_Pruned_Captioning_Model\model.pth', map_location=device)
    # Extract the state dictionary from the loaded model instance
    #state_dict = model_instance.state_dict()
    # Load the state dictionary into the model
    #model.load_state_dict(state_dict)
    
    model = torch.load('Only_ResNet_Pruned_Captioning_Model\model.pth', map_location=device)
    ...
    for epoch in range(num_epochs):
        ...
        if save_model:
            ...
            save_checkpoint(checkpoint , epoch , filename="C:\\Users\\jishu\\Documents\\Changwon\\ImageCaptioningTrial\\Image-Captioning-with-Transformers-main\\Pruning_Checkpoints\\TillDecoder_Pruning.pth")
